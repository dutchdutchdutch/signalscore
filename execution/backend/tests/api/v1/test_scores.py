...
# In-memory DB for rate limit test
_rate_limit_engine = create_engine(
    "sqlite:///:memory:",
    connect_args={"check_same_thread": False},
    poolclass=StaticPool,
)
_RateLimitSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=_rate_limit_engine)
...
def test_create_score_rate_limit(mock_score_company, mock_latest):
    ...
    # Point scoring_jobs at the test DB
    import app.services.scoring_jobs as scoring_jobs
    from app.models.scoring_job import ScoringJob  # noqa: ensure model registered
    Base.metadata.create_all(bind=_rate_limit_engine)
    scoring_jobs._session_factory = _RateLimitSessionLocal
    ...
    try:
        ...
        Base.metadata.drop_all(bind=_rate_limit_engine)
